...
update_config(cur_path + '/../experiments/fpn/cfgs/resnet_v1_101_visdrone_fpn_dcn_end2end_ohem.yaml')
...
def load_image_names(config):
    dataset_path = config.dataset_path
    test_image_set = config.test_image_set
    index_file = os.path.join(dataset_path, test_image_set, 'index.txt')
...
def load_image_path(config):
    dataset_path = config.dataset_path
    test_image_set = config.test_image_set

    return [os.path.join(dataset_path, test_image_set, 'images', x+'.jpg')
...
def main():
    ...
    arg_params, aux_params = load_param("/home/mcc/codes/Deformable-ConvNets/output/fpn/visdrone/resnet_v1_101_visdrone_fpn_dcn_end2end_ohem/train/fpn_visdrone", 7, process=True)

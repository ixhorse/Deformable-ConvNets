...
class resnet_v1_101_fpn_dcn_cascade(Symbol):
    def __init__(self):
        ...
        """
        shared_param_dict = ['cascade%d_offset_p%d' %(c+1, x) for x in range(2,5) for c in range(3)]
    ...
    def get_fpn_feature(self, c2, c3, c4, feature_dim=256):
        ...
        fpn_p4_1x1 = mx.symbol.Convolution(data=c4, kernel=(3, 3), pad=(1, 1), stride=(1, 1), num_filter=feature_dim, name='fpn_p4_1x1')
        fpn_p3_1x1 = mx.symbol.Convolution(data=c3, kernel=(3, 3), pad=(1, 1), stride=(1, 1), num_filter=feature_dim, name='fpn_p3_1x1')
        fpn_p2_1x1 = mx.symbol.Convolution(data=c2, kernel=(3, 3), pad=(1, 1), stride=(1, 1), num_filter=feature_dim, name='fpn_p2_1x1')
        # top-down connection
    ...
    def get_symbol(self, cfg, is_train=True):
        ...
        res2, res3, res4 = self.get_resnet_backbone(data, with_dpyramid=True, with_dconv=True)
        ...
        if is_train:
            ...
            aux_dict = {
                'op_type': 'pyramid_proposal', 'name': 'rois_rpn',
        ...
        with mx.name.Prefix('cascade%d_' % stage):
            if stage > 1:
                rois = mx.sym.Custom(rois=rois, bbox_pred=bbox_delta, im_info=im_info, name='rois', 
                                     cfg=cPickle.dumps(cfg), op_type='decode_bbox')
            if is_train:
                # ROI proposal target
    ...
    def init_weight(self, cfg, arg_params, aux_params):
        ...
        for name in self.arg_shape_dict.keys():
            if 'bbox_pred' in name:
                print name, self.arg_shape_dict[name]